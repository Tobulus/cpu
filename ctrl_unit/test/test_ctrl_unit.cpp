...
#define NOP_CYCLE() ctrl_unit->I_clk = 0;ctrl_unit->eval();
...
int main(int argc, char** argv, char** env) {
    ...
    CHECK(ctrl_unit->O_state, 1, "State should be 1 after I_reset");
    NOP_CYCLE();
    ...
    CHECK(ctrl_unit->O_state, 1, "State should be 1 but is %d", ctrl_unit->O_state);
    ...
    CHECK(ctrl_unit->O_state, 2, "State should be 1 but is %d", ctrl_unit->O_state);
    CHECK(ctrl_unit->O_execute, 0, "O_execute should be 0 but is %d", ctrl_unit->O_execute);
    NOP_CYCLE();
    ...
    ctrl_unit->I_clk = 1;
    ctrl_unit->eval();
    CHECK(ctrl_unit->O_state, 4, "State should be 4 but is %d", ctrl_unit->O_state);
    NOP_CYCLE();
    
    // Register read 
    ctrl_unit->I_clk = 1;
    ctrl_unit->eval();
    CHECK(ctrl_unit->O_state, 8, "State should be 8 but is %d", ctrl_unit->O_state);
    NOP_CYCLE();   
    ...
    ctrl_unit->I_clk = 1;
    ctrl_unit->eval();
    CHECK(ctrl_unit->O_state, 32, "State should be 32 but is %d", ctrl_unit->O_state);
    NOP_CYCLE();
